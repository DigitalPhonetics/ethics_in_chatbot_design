...
def exception_logging_hook(exc_type, exc_value, exc_traceback):
    ...
    logging.getLogger('adviser)survey').error("Uncaught exception",
...
class SurveyLogger(logging.Logger):
    ...
    def __init__(self, name: str = 'adviser_survey', console_log_lvl: LogLevel = LogLevel.ERRORS,
                 file_log_lvl: LogLevel = LogLevel.RESULTS, logfile_folder: str = 'logs',
                 logfile_basename: str = 'survey_log'):  # pylint: disable=too-many-arguments
        super(SurveyLogger, self).__init__(name)
        ...
        if file_log_lvl is not LogLevel.NONE:
            ...
            fh_formatter = MultilineFormatter('')
            file_handler.setFormatter(fh_formatter)
            self.addHandler(file_handler)

        if console_log_lvl is not LogLevel.NONE:
